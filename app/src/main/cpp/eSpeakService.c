//
// Created by Vinh Pham Xuan on 17/6/24.
//
/*
...
Java_ai_assistant_tts_SpeechSynthesis_nativeClassInit(JNIEnv *env, jclass clazz) {
    if (DEBUG) {
        LOGV("%s", __FUNCTION__);
    }
    jclass cls = (*env)->FindClass(env, "ai/assistant/tts/SpeechSynthesis");
    METHOD_nativeSynthCallback = (*env)->GetMethodID(env, cls, "nativeSynthCallback", "([B)V");
    if (METHOD_nativeSynthCallback == NULL) {
        if (DEBUG) {
            LOGV("Failed to get method ID for nativeSynthCallback");
        }
        return JNI_FALSE;  // Return false if the method ID is not found
    }

    return JNI_TRUE;  // Return true if initialization is successful
}

JNIEXPORT jint
JNICALL Java_ai_assistant_tts_SpeechSynthesis_nativeCreate(
...
JNIEXPORT jobject
JNICALL Java_ai_assistant_tts_SpeechSynthesis_nativeGetVersion(
...
JNICALL Java_ai_assistant_tts_SpeechSynthesis_nativeGetAvailableVoices(
...
JNIEXPORT jboolean
JNICALL Java_ai_assistant_tts_SpeechSynthesis_nativeSetVoiceByName(
...
JNIEXPORT jboolean
JNICALL Java_ai_assistant_tts_SpeechSynthesis_nativeSetVoiceByProperties(
...
JNIEXPORT jboolean
JNICALL Java_ai_assistant_tts_SpeechSynthesis_nativeSetParameter(
    ...
    switch (result) {
        case EE_OK:
            return JNI_TRUE;
        case EE_INTERNAL_ERROR:
            LOGE("espeak_SetParameter: internal error. %d", parameter);
            break;
        case EE_BUFFER_FULL:
...
JNIEXPORT jint
JNICALL Java_ai_assistant_tts_SpeechSynthesis_nativeGetParameter(
...
JNIEXPORT jboolean
JNICALL Java_ai_assistant_tts_SpeechSynthesis_nativeSetPunctuationCharacters(
...
JNIEXPORT jboolean
JNICALL Java_ai_assistant_tts_SpeechSynthesis_nativeSynthesize(
...
JNIEXPORT jboolean
JNICALL Java_ai_assistant_tts_SpeechSynthesis_nativeStop(
        JNIEnv *env, jobject object) {
    if (DEBUG) LOGV("%s", __FUNCTION__);
    espeak_Cancel();
